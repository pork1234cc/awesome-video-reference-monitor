...
description: 使用自包含工具登记并监控抖音或微信视频号对标账号；用户说“记录对标”并提供该账号任意作品链接，或说“监控对标”，或用“监控”描述点赞、收藏、评论、转发、转赞比、时间及本地/飞书模式时使用。不用于普通单链接“提取文案”，不评分、改写或创建定时任务。
---
...
用户没有指定存储模式时，使用项目根 `.env` 中的 `ARTICLEMONITOR_STORAGE_BACKEND`。用户在“记录对标”或“监控对标”请求中明确说“使用飞书模式”或“使用本地模式”时，将其分别映射为本次命令进程的 `ARTICLEMONITOR_STORAGE_BACKEND=feishu` 或 `ARTICLEMONITOR_STORAGE_BACKEND=local`，并在执行前回显所选模式。
...
- 用户明确发出“记录对标”或“监控对标”即授权本次 TikHub 查询和项目内约定目录的读写；安装依赖、删除数据或创建定时任务仍需另行授权。
